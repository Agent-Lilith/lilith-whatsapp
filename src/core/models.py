...
class Chat(Base):
    ...
    jid_pn: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
...
class Contact(Base):
    ...
    lid: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    push_name: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=func.now(),
...
class Message(Base):
    ...
    participant: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    participant_alt: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    remote_jid_alt: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    ...
    body_embedding: Mapped[Optional[Vector]] = mapped_column(Vector(EMBEDDING_DIM), nullable=True)

    chat: Mapped["Chat"] = relationship(back_populates="messages")

    __table_args__ = (UniqueConstraint("chat_id", "wa_message_id", name="uq_messages_chat_wa_message_id"),)
